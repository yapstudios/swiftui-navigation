  ...
  extension View {
    ...
    @ViewBuilder
    public func navigationDestinationBackported<Value, Destination: View>(
      ...
      if requiresBindWorkaround {
        ...
        self.nbNavigationDestination(isPresented: value.isPresent()) {
    ...
    ///     the destination.
    ///   - destination: A closure returning the content of the destination.
    public func navigationDestinationBackported<Enum, Case, Destination: View>(
      unwrapping enum: Binding<Enum?>,
      case casePath: CasePath<Enum, Case>,
      @ViewBuilder destination: (Binding<Case>) -> Destination
    ) -> some View {
      self.navigationDestinationBackported(unwrapping: `enum`.case(casePath), destination: destination)
    }
  }
  ...
  private struct _NavigationDestinationBindWorkaround<Destination: View>: ViewModifier {
    ...
    public func body(content: Content) -> some View {
      content
        .nbNavigationDestination(isPresented: self.$isPresentedState) { self.destination }
